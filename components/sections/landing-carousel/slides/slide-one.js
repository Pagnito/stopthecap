...
export default function SlideOne(/*{ data }*/) {
  ...
  return (
    <div className={`top-0 w-full bg-center bg-cover h-screen flex items-center sm:justify-end xs:pr-10 xs:pl-10 relative`}>
      <Image src="/images/fashion-one.jpg" alt="Landing Image" loading="eager" layout="fill" objectFit="cover" />
      <div className="p-5 mt-10 text-left text-white font-sans sm:mr-20 flex flex-col justify-start items-start">
        ...
        <Link href="/shop" passHref>
          <button
            className="animate-button opacity-0 -translate-x-8 hover:bg-red-500 transition-colors cursor-pointer 
        mt-12 pl-10 pr-10 pt-4 pb-4 text-lg border-solid border-red-500 border-2 self-start rounded"
          >
            SHOP NOW
          </button>
        </Link>
