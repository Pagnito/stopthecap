...
export default function Reviews(props) {
  let amountOfReviews = reviews.sku.length;
  ...
  let turnPage = (page) => {
    ...
    let newPaginationState = {
      reviews: reviews.sku.slice(page*4-4, page*4),
      page: page
    }
  ...
  let navigation = () => {
    ...
    return pagination.map((pageNumber) => {
      return (
        <div onClick={() =>turnPage(pageNumber + 1)} className={`${paginationState.page-1 == pageNumber ?'text-xl text-red-700' : ''} relative z-50 ml-3 cursor-pointer text-red-500`} key={pageNumber}>
          {pageNumber + 1}
        </div>
      );
    });
  ...
  return (
    <div ref={reviewsContainer} className="w-full p-10 rounded border-2">
